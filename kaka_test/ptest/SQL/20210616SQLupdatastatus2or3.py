...
def kaka_test_orderupdata():
  ...
  with conn.cursor() as cursor:
    ...
    for r in rows:
      print (1)
      #print (cmd,r)    
      cmd_fineidstatusone = "SELECT id FROM chenpays.lq_orders where status = 1;" 
      #print ("id",cmd_fineidstatusone)    
      cursor.execute(cmd_fineidstatusone)
      conn.commit()
      rs = cursor.fetchall()
      ...
      for r in rs:
        ...
        cmd_update="UPDATE `chenpays`.`lq_orders` SET `status` = '%s' WHERE (`id` = '%s');"%(ran,id)   
